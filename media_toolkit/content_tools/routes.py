...
def short_mobile():
    with current_app.app_context():
        # Ensuring correct prefixing within the app context
        # template_path = _with_prefix("content/short.html")
...
#     # print(f'prefix = {current_app.config.get("MEDIA_TOOLKIT_URL_PREFIX", "")}')
#     # exit()
#     # prefix = current_app.config.get("MEDIA_TOOLKIT_URL_PREFIX", "")
#     # return jsonify({"ok": True, "payload": f"{prefix} ===> {_with_prefix('page/dalej')}"})
#     return render_template(_with_prefix("content/short.html"), prompts=PROMPTS)
...
def apply_prompt():
    ...
    audiototext_logger.info(f'response ==> response len==>{len(response)}')
